...
class EntraIDOAuthProvider(
    ...
    def __init__(
        ...
        self.clients: dict[
            str, OAuthClientInformationFull
        ] = {}  # in-memory store for client information
        ...
        self.AUTHORIZATION_CODES = (
            {}
        )  # Temp store for authorization codes to used by MCP clients performing the authorization code flow
        self.ENTRAID_TOKENS = (
            {}
        )  # Temp store for tokens received from Entra ID (Capable of accessing Azure resources)
        self.TOKEN_MAPPING = (
            {}
        )  # Temp store for mapping Entra ID tokens to MCP tokens (MCP Clients (like agents) can use these tokens to access the MCP Server)
        self.TOKENS = (
            {}
        )  # Temp store for MCP tokens (MCP Clients can use these tokens to access the MCP Server)
    ...
    async def register_client(self, client_info: OAuthClientInformationFull) -> None:
        ...
        logger.info(
            f"Client {client_info.client_id} registered with redirect URIs: {client_info.redirect_uris}"
        )
    ...
    async def authorize(
        ...
        )  # Though one was already provided, we generate a new one for the redirect to Entra ID. This allows us to use our private code_verifier for the token exchange later.
    ...
    async def exchange_authorization_code(
        ...
        entra_token = self.ENTRAID_TOKENS[
            f"{authorization_code.code}{authorization_code.client_id}{authorization_code.code_challenge}"
        ]
        ...
        return OAuthToken(
            ...
            token_type="bearer",
            scope=Scope.from_list(
                authorization_code.scopes
            ).as_string(),  # may need to be adjusted, Is this the correct scope?
        )
